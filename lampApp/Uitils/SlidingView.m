...
//    NSLog(@"---------%@-------%@--------%f",NSStringFromCGPoint(point),NSStringFromCGRect(self.frame),_high);
    ...
    if(point.y < 0 && self.frame.origin.y + 7.5 <= 0){
        
    }else if(point.y > 0 && self.frame.origin.y + 7.5 >= _high){
    ...
    int proportion = ((_high - (self.frame.origin.y + 7.5)) / _high) * 100;
...
//    NSLog(@"---------%d",proportion);
    ...
    [self.delegate slidingImgViewDelegateFlot:self.frame.origin.y + 7.5 proportion:proportion idEnd:NO];
    
    if (panGestureRecognizer.state == UIGestureRecognizerStateEnded) {

        [self.delegate slidingImgViewDelegateFlot:self.frame.origin.y + 7.5 proportion:proportion idEnd:YES];
    }
}
@end
